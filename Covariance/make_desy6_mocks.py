...
import os, sys
import copy
sys.path.append('./Moments_analysis')
#from healpy_utils import convert_to_pix_coord, IndexToDeclRa, apply_random_rotation, addSourceEllipticity, gk_inv
import healpy as hp
import scipy
...
output = '/pscratch/sd/m/myamamot/sample_variance/v5_catalog/'
print(output)
...
def make_maps(seed):
    ...
    shell = np.load(path_sims+'/run_{0}//compressed_shells.npz'.format(rel)) #2048
    ...
    path_nz ='/pscratch/sd/m/myamamot/sample_variance/Y3_nontomographic_nz.txt'
    ...
    for zz in frogress.bar(range(len(z_bounds['z-low']))):
        shell_ = shell['shells'][i_sprt[zz]]
        #shell_ = shell_.astype(np.float32)*kappa_prefactor_


        # conversion from particle counts to convergence
        kappa_prefactor_ = UFalcon.lensing_weights.kappa_prefactor(
                            n_pix=hp.nside2npix(2048), n_particles=832**3,
    ...
    # *******************************
    maps_base[0] = conv_map_k
        
        
        
    print (maps_base.keys(), maps_base[0])
    ...
    del conv_map_k
    gc.collect()
    # mdet_catalog = load_obj('/global/cscratch1/sd/myamamot/sample_variance/data_catalogs_weighted_v3')
    import fitsio as fio
    mdet_catalog = fio.read('/pscratch/sd/m/myamamot/des-y6-analysis/y6_measurement/v5/metadetection_v5_flat_shape_err.fits')
    ...
    for i in range(1):
        ...
        dec1 = mdet_catalog['dec']
        ra1 = mdet_catalog['ra']
        w = mdet_catalog['w']
        R = mdet_catalog['R_all']
        print (len(w))
        
        es1,es2 = apply_random_rotation(mdet_catalog['g1']/R, mdet_catalog['g2']/R)
        ...
        es1,es2 = apply_random_rotation(mdet_catalog['g1'], mdet_catalog['g2'])
    ...
    save_obj(output+'seed_'+'_'+label_output+'_'+str(seed+1),{'sources':sources_cat}) ##,'lens':maps_base_lens,'randoms':random_maps})
