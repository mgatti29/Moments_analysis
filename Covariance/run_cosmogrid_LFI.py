...
def make_maps(seed):
    ...
    path_sims = path_sims_ +'/{0}'.format(params_dict['extra_map'])
    output_intermediate_maps = output_intermediate_maps_ +'/{0}/'.format(params_dict['extra_map'])
    output_temp = output_temp_ + '/{0}/'.format(params_dict['extra_map'])
    print (output_temp+p)
    '''
    ...
    #base_b = output_intermediate_maps+'/meta_{0}/'.format(config['f'])   
    ...
    if not os.path.exists(path_):
        ...
        # the following lines are commented because I don't want to save kappa to save space, but only g1 and g2.
        # you can remove the comment lines if you want to save it
        for s_ in range(kappa_lensing.shape[0]):
            #try:
            path_ = base_b+'/kappa_{0}_{1}.fits'.format(s_,config['nside_out'])
            #if not os.path.exists(path_):
            #    
            ##    if os.path.exists(path_):
            ##        os.remove(path_)
#
        ...
        for i in (range(kappa_lensing.shape[0])):
            ...
            if not os.path.exists(path_):
                ...
                g1_, g2_ = gk_inv(kappa_lensing[i]-np.mean(kappa_lensing[i]),kappa_lensing[i]*0.,config['nside_intermediate'],config['nside_intermediate']*3-1)
                g1_IA, g2_IA = gk_inv(overdensity_array[i]-np.mean(overdensity_array[i]),kappa_lensing[i]*0.,config['nside_intermediate'],config['nside_intermediate']*3-1)
    ...
    # free some memory in case these quantities have been used
    try:
    ...
    for tomo_bin in config['sources_bins']:
        ...
        g1_tomo_b[tomo_bin] = hp.pixelfunc.ud_grade(g1_tomo_b[tomo_bin],nside_out=nside_out)
        g2_tomo_b[tomo_bin] = hp.pixelfunc.ud_grade(g2_tomo_b[tomo_bin],nside_out=nside_out)                   
        d_tomo_b[tomo_bin] =  hp.pixelfunc.ud_grade( d_tomo_b[tomo_bin] ,nside_out=nside_out)
   ...
   # Add noise ++++++++++++++++++++++++++++++++=
    ...
    for tomo_bin in config['sources_bins']:   
       ...
       # of the catalogue galaxies in the same pixel . Last, it associates ellipticities based on w-ellipticity relation learned from the catalog
        ...
        sources_maps[tomo_bin]  = {'e1r':m_*e1r_map,'e2r':m_*e2r_map} 
    ...
    # save output 
    params_dict['random_rel'] = random_rel
    sources_maps['params_dict'] = params_dict
    save_obj(output_temp+p,sources_maps)


    
   #srun --nodes=4 --tasks-per-node=32 --cpus-per-task=2 --cpu-bind=cores  python run_cosmogrid_baryons.py
...
A_IA = 0.0
e_IA = 0.0
runs_cosmo = 1 # number of cosmogrid independent maps 
noise_type = 'random_depth' # or 'random_depth'


path_sims_ = '/global/cfs/cdirs/des/cosmogrid/raw/grid/'.format(0)
output_intermediate_maps_ = '/global/cfs/cdirs/des/mgatti/cosmogrid_tsz//'.format(0) # this is the fiducial run



# the final noisy maps will be saved here

output_temp_ = '/global/cfs/cdirs/des/mgatti//cosmogrid_tsz/'.format(0) 
...
if __name__ == '__main__':
    ...
    import glob
    fu_  = glob.glob('/global/cfs/cdirs/des/cosmogrid/raw/grid/*')
    for fu in fu_:
        extra_map = fu.split('/global/cfs/cdirs/des/cosmogrid/raw/grid/')[1]
    
        path_sims = path_sims_ +'/{0}'.format(extra_map)
        output_intermediate_maps = output_intermediate_maps_ +'/{0}/'.format(extra_map) 
        output_temp = output_temp_ + '/{0}/'.format(extra_map) 
        ...
        for f in range(runs_cosmo):
            ...
            for i in range(rot_num):
                ...
                for nn in range(noise_rels):
                    e_IA = np.random.randint(-500000,500000,1)[0]/100000
                    A_IA = np.random.randint(-300000,300000,1)[0]/100000
                    params_dict = dict()
                    ...
                    params_dict['extra_map'] = extra_map
                    params_dict['rot'] = i
                    ...
                    nuis = dict()
                    nuis['m'] = np.zeros(4) 
                    for tomo_bin in [0,1,2,3]:
                        m_ = config['m_sources'][tomo_bin-1]
                    ...
                    params_dict['m1'] =  nuis['m'][0]
                    params_dict['m2'] =  nuis['m'][1]
                    params_dict['m3'] =  nuis['m'][2]
                    params_dict['m4'] =  nuis['m'][3]
                    ...
                    p = str(f)+'_'+noise_type+'_'+str(Omegam )+'_'+str(s8)+'_'+str(ns)+'_'+str(Ob)+'_'+str(h )+'_w'+str(w0)+'_'+str(i+1)+'_noise_'+str(nn)
    ...
    while run_count<len(runstodo):
        ...
        comm.Barrier() 
   # '''
##srun --nodes=4 --tasks-per-node=32   python run_cosmogrid_LFI_fast.py
