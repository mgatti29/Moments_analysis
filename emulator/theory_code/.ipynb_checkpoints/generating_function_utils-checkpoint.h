...
double return_polnomial_value(double x, vector<double> *coefficients){
...
double get_delta_2_from_delta_1_nested(double w, double dmin, double dmax, double fmin, double fmax, double delta_L_1, double V_1, double R_L_1, double R_NL_2, vector<double> *delta_values, vector<double> *F_values, Matter* pointer_to_Matter){
  ...
  while(abs(delta_max-delta_min) > constants::eps_nested){
    ...
    F = interpolate_neville_aitken(delta_pivot, delta_values, F_values, pointer_to_Matter->order);
    if(F < -1.0){
    ...
    pointer_to_Matter->set_linear_Legendres(2, R_L_2/w/c_over_e5);
...
double get_delta_2_from_delta_1_final(double w, double d_start, double delta_L_1, double V_1, double R_L_1, double R_NL_2, vector<double> *delta_values, vector<double> *F_values, Matter* pointer_to_Matter){ 
  ...
  F = interpolate_neville_aitken(d, delta_values, F_values, pointer_to_Matter->order);
  F_prime = interpolate_neville_aitken_derivative(d, delta_values, F_values, pointer_to_Matter->order);
  if(F < -1.0){
    R_L_2 = 1.0/maximal_wave_number/c_over_e5;
    dR_L_2 = 0.5*F_prime/sqrt(1.0+(*F_values)[0])*R_NL_2/c_over_e5;
  }
  else{
    R_L_2 = R_NL_2*sqrt(1.0+F)/c_over_e5;
    dR_L_2 = 0.5*F_prime/sqrt(1.0+F)*R_NL_2/c_over_e5;
  }
  pointer_to_Matter->set_linear_Legendres(3, R_L_2/w/c_over_e5);
  
  V_12 = pointer_to_Matter->covariance_of_matter_within_R_2D();
  dV_12 = pointer_to_Matter->dcov2_at()/w*c_over_e5;
  f = d - alpha*V_12;
  ...
  while(abs(d_step) > constants::eps_Newton){
    ...
    if(F < -1.0){
      R_L_2 = 1.0/maximal_wave_number/c_over_e5;
      dR_L_2 = 0.0;//0.5*F_prime/sqrt(1.0+(*F_values)[0])*R_NL_2/c_over_e5;
    }
    else{
      R_L_2 = R_NL_2*sqrt(1.0+F)/c_over_e5;
      dR_L_2 = 0.5*F_prime/sqrt(1.0+F)*R_NL_2/c_over_e5;
...
double get_lambda_final(double delta, double lambda, vector<double> *G_coefficients, vector<double> *G_prime_coefficients){ 
  ...
  while(abs(lambda_now - lambda_previous)  > constants::eps_Newton){
    ...
    f = return_polnomial_value(lambda_now, G_coefficients)-delta;
    f_prime = return_polnomial_value(lambda_now, G_prime_coefficients);
...
double get_lambda_from_tau(double tau, double lambda, vector<double> *tau_coefficients, vector<double> *tau_prime_coefficients){ 
  ...
  while(abs(lambda_now - lambda_previous)  > constants::eps_Newton){
    ...
    f = return_polnomial_value(lambda_now, tau_coefficients)-tau;
    f_prime = return_polnomial_value(lambda_now, tau_prime_coefficients);
...
double get_tau_final(double delta, double tau, vector<double> *G_coefficients, vector<double> *G_prime_coefficients){ 
  ...
  while(abs(tau_now - tau_previous)  > constants::eps_Newton){
    ...
    f = return_polnomial_value(tau_now, G_coefficients)-delta;
    f_prime = return_polnomial_value(tau_now, G_prime_coefficients);
