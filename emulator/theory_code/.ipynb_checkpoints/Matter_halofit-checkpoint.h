...
double Matter::sig_sq(double R, double e){
	...
	double s_sq = 0;
  double prefactors;
  double D_sq = interpolate_neville_aitken(e, &this->eta_Newton, &this->Newtonian_growth_factor_of_delta, this->order);
  ...
  s_sq = int_gsl_integrate_medium_precision(halofit_sig_sq_gsl,(void*)pointer_to_params,log(minimal_wave_number),log(maximal_wave_number),NULL,len_ww);
...
vector<double> Matter::c_and_n_NL(double R, double e){
  ...
  double lnk_max = log(maximal_wave_number);
  double prefactors;
  double D_sq = interpolate_neville_aitken(e, &this->eta_Newton, &this->Newtonian_growth_factor_of_delta, this->order);  
  ...
  params.pointer_to_Matter = this;
	...
	integral[0] = 2.0*prefactors*int_gsl_integrate_medium_precision(halofit_C_gsl,(void*)pointer_to_params,log(minimal_wave_number),log(maximal_wave_number),NULL,len_ww);
  integral[1] = 4.0*prefactors*int_gsl_integrate_medium_precision(halofit_n_gsl,(void*)pointer_to_params,log(minimal_wave_number),log(maximal_wave_number),NULL,len_ww);
...
double Matter::Delta_Q_sq(double k, double e){
  ...
  //double Delta_L_sq = this->relativistic_linear_power_spectrum(k, e)*pow(k/c_over_e5, 3.0)/(2*pi_sq);
  double Delta_L_sq = this->Newtonian_linear_power_spectrum(k, e)*pow(k/c_over_e5, 3.0)/(2*pi_sq);
...
double Matter::P_NL_at(double k, double e){
  ...
  return (q+h)/pow(k/c_over_e5, 3)*(2*pi*pi);
...
double Matter::current_P_NL_at(double ln_k){
  ...
  return interpolate_neville_aitken(ln_k, &this->log_wave_numbers, &this->current_P_NL, this->order);
...
double Matter::current_P_L_at(double ln_k){
  ...
  return interpolate_neville_aitken(ln_k, &this->log_wave_numbers, &this->current_P_L, this->order);
...
vector<double> Matter::P_NL(double e){
  ...
  this->current_k_non_linear = k_s*c_over_e5;
  ...
  for(int i = 0; i < n; i++){
    k = this->wave_numbers[i];
		if(k > 0.5*c_over_e5)
			...
			P[i] = interpolate_neville_aitken_grid(z, k, &Mead_z_values, &Mead_k_values, &Mead_power_spectrum, this->order, this->order);
    }*/
...
vector<double> Matter::P_L(double e){
  ...
  for(int i = 0; i < n; i++){
    k = this->wave_numbers[i];
    P[i] = this->Newtonian_linear_power_spectrum(k, e);
  }
