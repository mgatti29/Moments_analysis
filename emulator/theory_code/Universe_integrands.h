...
int scale_factor_gsl(double scale, const double y[], double dfda[], void *params){
  ...
  dfda[0] = 1.0/(scale*hubble);
  dfda[1] = -0.5*(rho_m + 2*rho_r+(1.0+3.0*w)*rho_l)*scale/hubble;
  return GSL_SUCCESS;
}
...
int scale_factor_gsl_jac(double scale, const double y[], double *dfdy, double dfde[], void *params){
  ...
  gsl_matrix_set (m, 1, 1, -1.0/(scale*hubble*hubble));
  
  gsl_matrix_set (m, 2, 0, 0.0);
  gsl_matrix_set (m, 2, 1, 0.5*(rho_m + 2*rho_r+(1.0+3.0*w)*rho_l)*scale/pow(hubble, 2));
...
void Universe::set_number_of_entries(int n_entries){
  ...
  */
  ...
  this->number_of_entries = n_entries;
