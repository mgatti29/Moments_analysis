...
def execute(block, config):
    ...
    # *******************************************************
    # Read covariances
    cov_dprime = config['cov_2'] 
    cov_ddprime = config['cov12_cross']
    cov_d = config['cov_1']
    inv_cov_d = np.linalg.inv(cov_d)
    inv_cov_dprime = np.linalg.inv(cov_dprime)

    # compute theory moments *************
    if config['type_2'] == 'moments':
        theory_dprime = compute_th_moments(block,config['config_2']) #2nd
    else:
        pass

    if config['type_1'] == 'moments':
        theory_d = compute_th_moments(block,config['config_1'])
    else:
        pass
    config['theory_d'] = theory_d
    config['theory_dprime'] =theory_dprime
    # Read oberved DV
    
    
    obs_dprime = config['yobs_2']
    obs_d = config['yobs_1']
    
    save_obj('/project/projectdirs/des/mgatti/Moments_analysis/test_small_large',config)
    
    
    # compute realisations of possible d2 given observed d1
    mean_dprime_conditioned = theory_dprime + np.dot(cov_ddprime, np.dot(inv_cov_d, obs_d-theory_d))
    ...
    info_to_save = dict()
    info_to_save['chi2_realization']  = chi2_realizations
    info_to_save['chi2_data']  = chi2_data
    info_to_save['chi2_data0']  = chi2_data0
    info_to_save['dprime_realization']  = dprime_realization
    info_to_save['theory_dprime']  = theory_dprime
    info_to_save['theory_d']  = theory_d
    info_to_save['obs_dprime']  = obs_dprime
    info_to_save['cov_dprime'] = cov_dprime
    info_to_save['p_v'] = len(chi2_realizations[chi2_realizations>chi2_data])*1./len(chi2_realizations)
    print (chi2_realizations[0],chi2_data)
